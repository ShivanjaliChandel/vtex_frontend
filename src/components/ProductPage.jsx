    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                const skuResponse = await axios.get(`https://vtex-backend-94ev.onrender.com/sku/${skuId}`);
                setSku(skuResponse.data);

                // Fetch Pricing details
                // const pricingResponse = await axios.get(`http://localhost:3000/pricing/${skuId}`);
                const pricingResponse = await axios.get(`https://vtex-backend-94ev.onrender.com/pricing/${skuId}`);
                setPrice(pricingResponse.data.basePrice); // Extract `basePrice` from the pricing response

                // Fetch recommended products
                const recommendationsResponse = await axios.get(
                    `https://vtex-backend-94ev.onrender.com/recommendations/${skuId}`
